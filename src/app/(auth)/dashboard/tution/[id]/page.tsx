...
import ApproveButton from "../../[id]/_components/ApproveButton";

const Page = async ({ params }: { params: { id: string } }) => {
  const [tutId, userId] = params.id.split("-");
  const tution = await getSpecificTution(parseInt(tutId), parseInt(userId));

  if (tution.data?.status !== "ACCEPTED") {
    return (
      <h1 className="text-3xl font-bold text-center mt-10">
        Tution is not accepted yet (go and check your previous requirements
        page)
      </h1>
    );
  }
